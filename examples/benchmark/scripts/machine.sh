#!/usr/bin/env bash
# Capture the machine identity for a benchmark run. Absolute build and startup
# times depend on the hardware, so every published number should carry this
# context. Writes machine.txt into the raw directory.
...
mem_total() {
  if [ -r /proc/meminfo ]; then awk '/MemTotal/ { printf "%.1f GB\n", $2/1024/1024 }' /proc/meminfo
...
out="$RAW_DIR/machine.txt"
{
  ...
  echo "Kernel:      $(uname -r)"
  echo "gopack:      $("$GOPACK_BIN" version 2>/dev/null || echo 'n/a')"
